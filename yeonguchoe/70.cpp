...
int ways_to_make_reamining(int remaining, int goal) {
    if (remaining == 0) {
        return 1;
    } else if (remaining < 0) {
        return 0;
    }
    return ways_to_make_reamining(remaining - 1, goal) +
           ways_to_make_reamining(remaining - 2, goal);
}

int climbStairs(int n) {
    return ways_to_make_reamining(n, n);
}
...
int ways_to_make_reamining(int remaining, int goal, int* memoization) {
    ...
    memoization[remaining] =
        ways_to_make_reamining(remaining - 1, goal, memoization) +
        ways_to_make_reamining(remaining - 2, goal, memoization);
    return memoization[remaining];
}
...
int climbStairs(int n) {
    ...
    return ways_to_make_reamining(n, n, memo);
}
